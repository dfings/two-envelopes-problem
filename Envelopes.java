...
public final class Envelopes {
    ...
    /**
     * Runs a single trial where an envelope is chosen. If the chosen envelope has a
     * value &lt; cutoff, the function will switch envelopes, otherwise it will keep
    ...
    private static double singleTrial(Picker picker) {
        ...
        return random.nextInt(2) == 0 ? picker.pick(lowerValue, higherValue) : picker.pick(higherValue, lowerValue);
    }
    ...
    public static void main(String[] args) {
        for (int cutoff = 0; cutoff <= 2 * LOWER_PRIOR_MAX; ++cutoff) {
            System.out.println(String.format("cutoff=%d, expected_value=%f", cutoff, multiTrial(cutoff)));
        }
    }
}